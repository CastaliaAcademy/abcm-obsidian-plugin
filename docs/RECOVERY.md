...
The status becomes `auth-required` and synchronization pauses without deleting vault files, cursor, or outbox. Create a new server-side pairing code, select **Re-pair device**, and pair again. The plugin retains the assigned scope while authorization is cleared: pairing the same scope resumes the durable state, while a grant for another workspace, project, or prefix resets that state before the new credential is stored. Persisted sync state is also bound to that exact scope, so an unbound legacy state or mismatched scope starts a new confirmed initial preview instead of reusing its cursor. Do not paste an administrative ABCM token into plugin settings.
